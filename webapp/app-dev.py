...
with st.sidebar:
    ...
    # For the peer comparison, show all topics regardless if they are in commmon or not
    # OPTION 1
    selected_topics = sorted(
    merged[merged["company_name"].isin(selected)]["topic_label"]
    .dropna()
    .unique()
    .tolist()
)
    ...
    if not selected_topics:
        st.warning("No topics available for the selected companies.")
        st.stop()

    sel_topic = st.selectbox("Topic filter", selected_topics)
...
with tab1:
    ...
    with c2:
        ...
        t1_topic_n = safe_n_slider(ctrl4, "N topics", max_topics, 5, "t1_topic_n")
...
with tab2:
    ...
    company_topics = sorted(co_data["topic_label"].dropna().unique().tolist())
    if not company_topics:
        st.info("No topics available for this company.")
    else:
        t2_topic = st.selectbox("Topic", company_topics, format_func=dn, key="t2_topic")
    ...
    trend = _period_df(
        co_data[co_data["topic_label"] == t2_topic]
    ...
    if len(trend) and t2_trend_view in ["Both", "Topic only"]:
        fig_t.add_trace(go.Scatter(
            ...
            name=dn(t2_topic), mode="lines+markers",
...
with tab3:
    ...
    if len(selected) < 2:
        ...
        if not peer_topics:
            ...
            default_idx = peer_topics.index(sel_topic) if sel_topic in peer_topics else 0
            ...
            st.caption(f"Shared topic: {dn(peer_topic)}")
            ...
            peer_rank = ranked_company_scores(selected, sel_year, sel_q, topic=peer_topic)
            ...
            if len(peer_rank) == 0:
                ...
                peer_plot_df = (
                    merged[
                        ...
                        (merged["topic_label"] == peer_topic)
                        ][["company_name", "year", "quarter", "sentiment_score"]]
                        .copy()
                )
                ...
                peer_plot_df = (
                    peer_plot_df
                    .groupby(["company_name", "year", "quarter", "q_num"], as_index=False)["sentiment_score"]
                    .mean()
                )

                peer_plot_df["period_key"] = peer_plot_df["year"] * 10 + peer_plot_df["q_num"]
                peer_plot_df["period"] = peer_plot_df["year"].astype(str) + " " + peer_plot_df["quarter"]
                ...
                fig_peer = go.Figure()
                for c in peer_companies:
                    sub = peer_plot_df[peer_plot_df["company_name"] == c].sort_values("period_key")
                    if not len(sub):
                        continue
                    fig_peer.add_trace(go.Scatter(
                        x=sub["period"],
                ...
                peer_rank_mode = rctrl1.selectbox(
                    "Ranking filter", ["All", "Top", "Bottom"], key="peer_rank_mode"
                )
                max_rank_n = max(1, min(10, len(selected)))
                peer_rank_n = safe_n_slider(
                    rctrl2, "N companies (ranking)", max_rank_n, 5, "peer_rank_n"
                )

                rank_df = ranked_company_scores(selected, sel_year, sel_q, topic=peer_topic)
                if len(rank_df):
                    rank_df = apply_top_bottom(rank_df, peer_rank_mode, peer_rank_n).sort_values(
            ...
            yoy_rows = []
            for c in selected:
                now_sub = _filter(merged, company=c, year=sel_year, quarter=sel_q, topic=peer_topic)
                prev_sub = _filter(merged, company=c, year=(sel_year - 1 if sel_year else None), quarter=sel_q, topic=peer_topic)

                now = float(now_sub["sentiment_score"].mean()) if len(now_sub) else np.nan
                prev = float(prev_sub["sentiment_score"].mean()) if len(prev_sub) else np.nan
...
with tab4:
    ...
    if not len(fd):
        ...
        if len(topic_df):
            ...
            t4_topic_n = safe_n_slider(fctrl2, "N topics (filing)", max_t4, 5, "t4_topic_n")
    ...
    if not len(tm_df):
        ...
        if not len(topic_stats):
            ...
            with right_tm:
                ...
                top_topic_n = safe_n_slider(st, "Topics in distribution", max_dist, 6, "tm_dist_n")
